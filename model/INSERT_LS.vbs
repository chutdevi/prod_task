...
dim ej, cn, path, log, filename, filedata, fileins, namelog, tab
	...
	filename = "INSERT_LS"
	filedata = "PROD_DAT_LAST.sql"
	fileins  = "PROD_INS.sql"
	namelog = "INSERT_LS_InsLog"
	tab = "PROD_MODEL_PHASE8"
...
If Not fso.FileExists( path & "\log\Log.log" ) Then	
	...
	Set objFile= fso.CreateTextFile(outFile,True)
		objFile.WriteLine "[ LOG FOR TASK AUTOMATIC PRODUCTION GROUP MODEL REPORT ]"
		objFile.Close
END IF
